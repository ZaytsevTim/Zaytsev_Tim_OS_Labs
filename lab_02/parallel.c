...
void child_process(process_data_t data, shared_data_t* shared) {
    ...
    unsigned int seed = time(NULL) + data.process_id + getpid();
...
int main(int argc, char **argv) {
    ...
    if (argc > 1 && strcmp(argv[1], "-m") == 0) {
        if (argc < 3) {
            char msg[1024];
            uint32_t len = snprintf(msg, sizeof(msg) - 1, "Error: -m needs a value\n");
    ...
    if (argc - skip_args < 1) {
        char msg[1024];
        uint32_t len = snprintf(msg, sizeof(msg) - 1, "usage: %s K tour_number player1_pts player2_pts amount_of_experiments amount_of__threads\n", argv[0]);
        write(STDERR_FILENO, msg, len);
        return 1;
    }
    ...
    int ths = string_to_int(argv[6]);

    parallel_version(K, tour, pts1, pts2, exs, ths);
    return 0;
}